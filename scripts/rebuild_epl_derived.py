...
print("REBUILDING EPL DERIVED FILES SAFELY")
print("MATCHES ARE READ ONLY")
...
for path in sorted(match_files):
    ...
    match_id = clean(game.get("match_id") or os.path.basename(path).replace(".json", ""))
    season = clean(game.get("season") or os.path.basename(os.path.dirname(path)))
...
for slug, pdata in sorted(players.items()):
    ...
    with open(f"{TMP_PLAYERS_DIR}/{slug}.json", "w", encoding="utf-8") as f:
...
teams_out = sorted(teams.values(), key=lambda x: x["team"])
...
for team, data in sorted(team_stats.items()):
    team_stats_out.append({
        ...
        "top_scorers": [
            {"player": p, "goals": g}
            for p, g in sorted(data["top_scorers"].items(), key=lambda x: (-x[1], x[0]))[:20]
            if g > 0
        ],
        "yellow_cards": [
            {"player": p, "yellow": y}
            for p, y in sorted(data["yellow_cards"].items(), key=lambda x: (-x[1], x[0]))[:20]
            if y > 0
        ],
        "red_cards": [
            {"player": p, "red": r}
            for p, r in sorted(data["red_cards"].items(), key=lambda x: (-x[1], x[0]))[:20]
            if r > 0
        ]
    })
