...
def main():
    ...
    for file_path in sorted(match_files):
        ...
        for card in match.get("yellow_cards") or []:
            ...
            if not name:
                continue
            team = resolve_team(card.get("team") or "", name, home_team, away_team, known_player_teams)
            if not team:
                continue
        ...
        for scorer in match.get("scorers") or []:
            ...
            if not name:
                continue
            team = resolve_team(scorer.get("team") or "", name, home_team, away_team, known_player_teams)
            if not team:
                continue
            known_player_teams[name] = team
