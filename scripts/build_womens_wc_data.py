#!/usr/bin/env python3
"""
build_womens_wc_data.py

Reads all match JSON files from docs/data/womens-wc/matches/<year>/*.json
and generates three output files:
  - docs/data/womens-wc/players.json
  - docs/data/womens-wc/teams.json
  - docs/data/womens-wc/team-stats.json
...
OUT_DIR     = REPO_ROOT / "docs" / "data" / "women's-wc" / "womens-world-cup"
...
def tournament_from_path(file_path: Path) -> str:
    """Return the tournament year folder (e.g. '2023') from the file path."""
...
def make_team_stats(team: str, tournament: str) -> dict:
    return {
        "team": team,
        "season": tournament,   # kept as "season" for front-end compatibility
        "played": 0,
        "wins": 0,
        "draws": 0,
        "losses": 0,
...
def get_or_create_team_stats(team_stats_map, team, tournament):
    key = f"{team}|{tournament}"
    if key not in team_stats_map:
        team_stats_map[key] = make_team_stats(team, tournament)
    return team_stats_map[key]


def get_or_create_player(players_map, name):
    """Always key by name alone — one record per player regardless of tournament."""
    if name not in players_map:
        players_map[name] = make_player(name)
    return players_map[name]


def get_or_create_player_season(player, tournament, team):
    for s in player["season_stats"]:
        if s["season"] == tournament and s["team"] == team:
            return s
    entry = {
        "season": tournament,
        "team": team,
...
def add_season_if_missing(player, tournament):
    if tournament and tournament not in player["seasons"]:
        player["seasons"].append(tournament)
...
def main():
    ...
    for file_path in match_files:
        ...
        tournament = tournament_from_path(file_path)
        ...
        for team, scored, conceded in [
            ...
            s = get_or_create_team_stats(team_stats_map, team, tournament)
            s["played"]        += 1
            s["goals_for"]     += scored
        ...
        for card in match.get("yellow_cards") or []:
            ...
            if name:
                ...
                add_season_if_missing(p, tournament)
                p["yellow_cards"] += 1
                ps = get_or_create_player_season(p, tournament, team)
                ps["yellow_cards"] += 1
